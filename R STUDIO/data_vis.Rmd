...
vars = c("I_EN", "Q_EN", "MCOST_INV", "K_EN","Q_EMI", "BAU_Q_EMI", 
         ...
         "Q_OUT","Q_OUT","I_RD","TEMP","wcum","battery_cost_new")
...
```{r}
chosen_countrys = c("europe","china")
...
```
```{r}
ggsave(filename = "battery price.png", plot = plot, width = 15, height = 10, units = "cm") # A4 format
```
...
plot <- ggplot(data, aes(x = x)) +
  geom_line(aes(y = y1), color = "red", linetype = "solid", size = 1) +
  geom_line(aes(y = y2), color = "blue", linetype = "dashed", size = 1) +
  geom_text(aes(x = 5.2, y = 25, label = "Curva 1"), color = "red", vjust = -0.5) +
  geom_text(aes(x = 5.2, y = 16, label = "Curva 2"), color = "blue", vjust = -0.5) +
  xlab("x") +
  ylab("y") +
  theme_minimal()
